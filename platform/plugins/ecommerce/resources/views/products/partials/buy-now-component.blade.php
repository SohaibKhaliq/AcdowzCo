...
<div class="buy-now-component" data-product-id="{{ $product->id ?? '' }}">
    @if($showQuantitySelector)
        <div class="flex items-center space-x-4 mb-4">
            <label for="quantity-{{ $product->id ?? 'default' }}" class="text-sm font-medium text-gray-700">Quantity:</label>
            <div class="flex items-center border border-gray-300 rounded-md">
                <button type="button" class="quantity-btn decrease p-2 text-gray-500 hover:text-gray-700" data-action="decrease">
                    <i class="fas fa-minus text-xs"></i>
                </button>
                <input 
                    type="number" 
                    id="quantity-{{ $product->id ?? 'default' }}" 
                    name="quantity" 
                    value="1" 
                    min="1" 
                    ...
                    readonly
                >
                <button type="button" class="quantity-btn increase p-2 text-gray-500 hover:text-gray-700" data-action="increase">
    ...
    <!-- Buy Now Modal -->
    <div id="buy-now-modal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="border rounded-lg p-4">
                                    ...
                                    <form id="guest-checkout-form" class="space-y-3">
                                        <div>
                                            <input type="email" id="guest-email" name="guest_email" placeholder="Your email address" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent" required>
                                        </div>
                                        <div>
                                            <input type="text" id="guest-name" name="name" placeholder="Your full name" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent">
                                        </div>
                                ...
                                <div class="border rounded-lg p-4">
                                    ...
                                    <div id="phone-login-step">
                                        <form id="phone-login-form" class="space-y-3">
                                            <div>
                                                <input type="tel" id="phone-number" name="phone" placeholder="+1 (555) 123-4567" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                                            </div>
                                    ...
                                    <div id="phone-verify-step" class="hidden">
                                        <form id="phone-otp-verify-form" class="space-y-3">
                                            <div>
                                                <input type="text" id="otp-code-modal" name="code" maxlength="6" placeholder="Enter 6-digit code" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-center text-xl tracking-widest" required>
                                            </div>
                                            <button type="submit" class="w-full bg-green-500 text-white py-2 px-4 rounded-md hover:bg-green-600 transition-colors">
                                                <i class="fas fa-check mr-2"></i>
                                                Verify & Checkout
                                            </button>
                                            <button type="button" id="back-to-phone" class="w-full text-blue-600 hover:text-blue-800 text-sm">
                                                ← Change phone number
                                            </button>
                                        </form>
                                    </div>
                                ...
                                <div class="border rounded-lg p-4">
                                    <a href="{{ route('auth.google') }}?redirect=buy-now&product_id={{ $product->id ?? '' }}" class="w-full flex items-center justify-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors">
                ...
                <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                    <button type="button" id="close-modal" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:w-auto sm:text-sm">
                        Cancel
                    </button>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    phoneVerifyForm.addEventListener('submit', async function(e) {
        ...
        try {
            // First verify OTP
            const verifyResponse = await fetch('{{ route("api.ecommerce.verify-phone-otp") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
